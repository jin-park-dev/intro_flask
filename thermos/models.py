...
class Bookmark(db.Model):
    ...
    #Tag needs to be string because tag isn't known yet
    _tags = db.relationship('Tag', secondary=tags, backref=db.backref('bookmarks', lazy='dynamic'))

    @staticmethod
    def newest(num): #Modifies query but doesn't execute it.
        return Bookmark.query.order_by(desc(Bookmark.date)).limit(num)

    #Giant long string containing "tag for specific bookmark"
    @property
    def tags(self):
